...
- (void) sendFrame:(JCFLVVideoFrame*)frame{
...
- (void)sendFrame{
    if(!self.isSending && self.buffer.list.count > 0){
        ...
        // 调用发送接口
        JCFLVVideoFrame *frame = [self.buffer popFirstObject];
        if([frame isKindOfClass:[JCFLVVideoFrame class]]){
            if(!self.sendVideoHead){
                self.sendVideoHead = YES;
                [self sendVideoHeader:(JCFLVVideoFrame*)frame];
            }else{
                [self sendVideo:(JCFLVVideoFrame*)frame];
            }
        }
    }
}
...
- (void)sendVideo:(JCFLVVideoFrame*)frame{
